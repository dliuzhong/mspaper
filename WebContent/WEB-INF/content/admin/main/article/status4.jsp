  ...
  <body>
    	...
    	new Ext.onReady(function() {
    		...
    		var remoteJsonStore = new Ext.data.JsonStore({
    			...
    			url		:	'http://localhost/pw/json/getstatusArticlesJson.action?status=4',
    			
    			totalProperty	:	'totalCount',
    			root	:	'status',
    			id		:	'status4RemoteStore',
    			autoLoad	:	false,
    			remoteSort	:	true
    		});
    		
    		var recordField = [
    			'id', 'title', 'msuser', 'keyword', 'content', 'type', 'time', 'checktime', 'admin', 'publish', 'other', 'see'
    		];
    		...
    		Ext.get('export').on('click', function() {
         	...
         	Ext.get('publish').on('click', function() {
             	var selections = grid.getSelectionModel().getSelections();    //先获取选择模型，然后从选择模型中获取选中的记录。
             	if (selections.length == 1) {
             		for (var i = 0; i < selections.length; i++) {
                		var record = selections[i];
    								...
    								handler	:	function() {
										...
										form.getForm().submit({
    										...
    										success : function(form, action) {
                            					...
                            					Ext.MessageBox.alert('消息','提交成功！', function closeWin() {var win = Ext.getCmp('formWindow');win.close(); location.reload();});
    		...
    		Ext.StoreMgr.get('status4RemoteStore').load({
    			params	:	{
    				start	:	0,
    				limit	:	20
    			}
